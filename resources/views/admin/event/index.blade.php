...
<div class="wrap">
    ...
    @php
        $sortLink = function (string $column, string $label) use ($sortBy, $sortDir, $search, $perPage) {
            ...
            $url = route('admin.events', array_filter([
                'q'        => $search,
                'sort'     => $column,
                'dir'      => $nextDir,
    ...
    <div class="toolbar">
        <form action="{{ route('admin.events') }}" method="GET" class="search-form" id="eventSearchForm">
            @if ($sortBy !== 'event_date') <input type="hidden" name="sort" value="{{ $sortBy }}"> @endif
            ...
            <input type="text" name="q" id="eventSearchInput" value="{{ $search }}"
                   placeholder="Search by title, venue or description..." autocomplete="off">
            @if ($search !== '')
                <a href="{{ route('admin.events', array_filter(['sort' => $sortBy !== 'event_date' ? $sortBy : null, 'dir' => $sortDir !== 'asc' ? $sortDir : null, 'per_page' => $perPage !== 10 ? $perPage : null])) }}" class="search-clear" title="Clear search">
        ...
        <div class="toolbar-right">
            ...
            <form action="{{ route('admin.events') }}" method="GET" class="perpage-form" id="eventPerPageForm">
                @if ($search !== '') <input type="hidden" name="q" value="{{ $search }}"> @endif
                @if ($sortBy !== 'event_date') <input type="hidden" name="sort" value="{{ $sortBy }}"> @endif
                @if ($sortDir !== 'asc') <input type="hidden" name="dir" value="{{ $sortDir }}"> @endif
                <label for="eventPerPageSelect">Show</label>
                <select name="per_page" id="eventPerPageSelect" onchange="this.form.submit()">
        ...
        @if ($events->isEmpty())
            <div class="empty-state">
                <div class="ico-circle" style="width:52px;height:52px;margin:0 auto 12px;">
                    <i class="bi bi-calendar-event" style="color:#AEB4C4;font-size:22px;"></i>
                </div>
                @if ($search !== '')
                    <p>No events match "<b>{{ $search }}</b>".</p>
                    <a href="{{ route('admin.events') }}" class="choose-btn" style="display:inline-block;width:auto;padding:9px 20px;text-decoration:none;">Clear search</a>
                @else
                    <p>No events added yet.</p>
                    <a href="{{ route('admin.events.create') }}" class="choose-btn" style="display:inline-block;width:auto;padding:9px 20px;text-decoration:none;">Add the first event</a>
                @endif
            </div>
        @else
    ...
    @if ($events->hasPages())
        @php
            $pageUrl = function (int $page) use ($search, $sortBy, $sortDir, $perPage) {
                return route('admin.events', array_filter([
                    'q'        => $search,
                    'sort'     => $sortBy !== 'event_date' ? $sortBy : null,
        ...
        <nav class="pager" role="navigation" aria-label="Pagination">
            <div class="pager-status">
                ...
                of <b>{{ $events->total() }}</b> {{ Str::plural('result', $events->total()) }}
            </div>
...
<style>
    ...
    .row-thumb{ width:40px; height:40px; border-radius:8px; object-fit:cover; display:block; }
    .row-thumb-placeholder{ background:#EEF0F6; display:flex; align-items:center; justify-content:center; color:#AEB4C4; font-size:15px; }
