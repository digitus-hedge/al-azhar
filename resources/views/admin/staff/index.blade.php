...
@php
    // Defaults must match StaffController@index
    $defaultSort    = 'id';
    $defaultDir     = 'desc';
    $defaultPerPage = 10;

    // URL for this page keeping current state; defaults are dropped to keep URLs short
    $listUrl = function (array $overrides = []) use ($search, $sortBy, $sortDir, $perPage, $defaultSort, $defaultDir, $defaultPerPage) {
        $params = array_merge([
            'q'        => $search,
            'sort'     => $sortBy !== $defaultSort ? $sortBy : null,
            'dir'      => $sortDir !== $defaultDir ? $sortDir : null,
            'per_page' => $perPage !== $defaultPerPage ? $perPage : null,
        ], $overrides);

        return route('admin.staff', array_filter($params, fn ($v) => $v !== null && $v !== ''));
    };
...
<div class="wrap">
    ...
    <div class="toolbar">
        <form action="{{ route('admin.staff') }}" method="GET" class="search-form" id="staffSearchForm">
            @if ($sortBy !== $defaultSort) <input type="hidden" name="sort" value="{{ $sortBy }}"> @endif
            @if ($sortDir !== $defaultDir) <input type="hidden" name="dir" value="{{ $sortDir }}"> @endif
            @if ($perPage !== $defaultPerPage) <input type="hidden" name="per_page" value="{{ $perPage }}"> @endif
                ...
                <table class="staff-table">
                    <thead>
                        <tr>
                            ...
                            <th>{!! $sortLink('department_id', 'Department') !!}</th>
                            <th>{!! $sortLink('has_login', 'Login Access') !!}</th>
                    ...
                    <tbody>
                        @foreach ($staff as $member)
                            <tr>
                                <td data-label="Photo">
                                    <a href="{{ route('admin.staff.show', $member) }}" class="thumb" title="View details">
                                        @if ($member->photo)
                                ...
                                <td data-label="Name">
                                    <a href="{{ route('admin.staff.show', $member) }}" class="name-link"><b>{{ $member->name }}</b></a>
                                ...
                                <td data-label="Department">{{ $member->department?->name ?? '—' }}</td>
                                ...
                                <td data-label="Actions" style="text-align:right;white-space:nowrap;">
                                    <a href="{{ route('admin.staff.show', $member) }}" class="icon-btn icon-btn-view" title="View">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    ...
                                    <button type="button" class="icon-btn icon-btn-danger" title="Delete"
                                            onclick="confirmDeleteStaff({{ $member->id }}, @js($member->name))">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                    <form id="delete-form-{{ $member->id }}" action="{{ route('admin.staff.destroy', $member) }}"
        ...
        <nav class="pager" role="navigation" aria-label="Pagination">
            ...
            @if ($staff->hasPages())
                <div class="pager-links">
                    ...
                    @else
                        <a href="{{ $staff->url($current - 1) }}" class="pager-btn" rel="prev"><i class="bi bi-chevron-left"></i></a>
                    @endif

                    @if ($start > 1)
                        <a href="{{ $staff->url(1) }}" class="pager-btn">1</a>
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $staff->url($page) }}" class="pager-btn">{{ $page }}</a>
                        @endif
                    @endfor

                    @if ($end < $last)
                        @if ($end < $last - 1)<span class="pager-dots">&hellip;</span>@endif
                        <a href="{{ $staff->url($last) }}" class="pager-btn">{{ $last }}</a>
                    @endif

                    @if ($current >= $last)
                        <span class="pager-btn pager-btn-disabled" aria-disabled="true"><i class="bi bi-chevron-right"></i></span>
                    @else
                        <a href="{{ $staff->url($current + 1) }}" class="pager-btn" rel="next"><i class="bi bi-chevron-right"></i></a>
                    @endif
                </div>
            @endif
        </nav>
...
<style>
    ...
    .name-link{ color:inherit; text-decoration:none; margin-right:6px; }
    ...
    .icon-btn-view:hover{ background:#EEF2FF; color:#3538CD; border-color:#C7D2FE; }
    ...
    @media (max-width: 900px){
        ...
        .staff-table th, .staff-table td{ padding:12px 14px; }
    }
    ...
    @media (max-width: 640px){
        ...
        /* [photo] [name + head]
           details ...
           [view] [edit] [delete]  (full-width action row) */
        .staff-table tr{
            display:grid;
        ...
        .staff-table td{ display:block; padding:0; border:none; }

        .staff-table td[data-label="Photo"]       { grid-area:photo; }
        .staff-table td[data-label="Name"]        { grid-area:name; min-width:0; }
        .staff-table td[data-label="Designation"] { grid-area:desg; }
        .staff-table td[data-label="Department"]  { grid-area:dept; }
        .staff-table td[data-label="Login Access"]{ grid-area:login; }
        .staff-table td[data-label="Actions"]     { grid-area:actions; }

        .thumb{ width:48px; height:48px; border-radius:10px; }
        .name-link{ display:block; margin:0 0 4px; }
        .name-link b{ font-size:14px; }
        .badge-head{ font-size:10.5px; padding:2px 8px; }

        /* Label on the left, value on the right */
        .staff-table td[data-label="Designation"],
        .staff-table td[data-label="Department"],
        .staff-table td[data-label="Login Access"]{
            display:flex; justify-content:space-between; align-items:center; gap:12px;
            font-size:13px; padding-top:8px; border-top:1px dashed var(--line,#E9EBF2);
        }
        .staff-table td[data-label="Designation"]::before,
        .staff-table td[data-label="Department"]::before,
        .staff-table td[data-label="Login Access"]::before{
            content:attr(data-label); font-size:11px; font-weight:700; text-transform:uppercase;
            letter-spacing:.04em; color: var(--faint,#9AA1B2); flex-shrink:0;
        }

        /* Three equal action buttons */
        .staff-table td[data-label="Actions"]{
            display:grid; grid-template-columns:repeat(3, 1fr); gap:8px;
            padding-top:10px; border-top:1px dashed var(--line,#E9EBF2);
        }
        .staff-table td[data-label="Actions"] form{ display:none; }
        .staff-table td[data-label="Actions"] .icon-btn{ width:100%; height:38px; margin:0; }
    ...
    @media (max-width: 380px){
        ...
        .staff-table tr{ grid-template-columns:40px 1fr; }
