...
@php
    ...
    $listUrl = function (array $overrides = []) use ($search, $sortBy, $sortDir, $perPage, $trashed, $type) {
        $params = array_merge([
            'q'        => $search,
            'type'     => $type,
        ...
        return route('admin.designations', array_filter($params, fn ($v) => $v !== null && $v !== ''));
    };
...
<div class="wrap">
    ...
    <div class="header">
        <div>
            <h1>Designations</h1>
            <p>Management designations (Chairman, Secretary, Trustee) for School Management profiles, and Staff designations (Principal, PGT, Librarian) for the Staff form.</p>
        </div>
        <a href="{{ route('admin.designations.create', array_filter(['type' => $type])) }}" class="btn-save" style="text-decoration:none;">
            <i class="bi bi-plus-lg"></i>
            Add {{ $type ? $types[$type] : 'Designation' }}
        </a>
    </div>
    ...
    <div class="tabs">
        <a href="{{ $listUrl(['type' => null, 'page' => null]) }}" class="tab {{ ! $type ? 'on' : '' }}">
            ...
            <span class="tab-count">{{ $typeCounts->sum() }}</span>
        </a>
    ...
    <div class="toolbar">
        <form action="{{ route('admin.designations') }}" method="GET" class="search-form" id="desSearchForm">
            ...
            <input type="text" name="q" id="desSearchInput" value="{{ $search }}"
                   placeholder="Search designations..." autocomplete="off">
            @if ($search !== '')
        ...
        <div class="toolbar-right">
            <div class="toolbar-meta">
                {{ $designations->total() }} {{ \Illuminate\Support\Str::plural('designation', $designations->total()) }}
        ...
        @if ($designations->isEmpty())
            <div class="empty-state">
                <div class="ico-circle" style="width:52px;height:52px;margin:0 auto 12px;">
                    <i class="bi bi-person-badge" style="color:#AEB4C4;font-size:22px;"></i>
                </div>
                @if ($search !== '')
                    <p>No designations match "<b>{{ $search }}</b>".</p>
                    <a href="{{ $listUrl(['q' => null]) }}" class="choose-btn">Clear search</a>
                @elseif ($trashed)
                    <p>Trash is empty.</p>
                @else
                    <p>No {{ $type ? strtolower($types[$type]) . 's' : 'designations' }} added yet.</p>
                    <a href="{{ route('admin.designations.create', array_filter(['type' => $type])) }}" class="choose-btn">Add the first designation</a>
                @endif
            </div>
        @else
        ...
        <nav class="pager" role="navigation" aria-label="Pagination">
            ...
            @if ($designations->hasPages())
                <div class="pager-links">
                    ...
                    @else
                        <a href="{{ $designations->url($current - 1) }}" class="pager-btn" rel="prev"><i class="bi bi-chevron-left"></i></a>
                    @endif

                    @if ($start > 1)
                        <a href="{{ $designations->url(1) }}" class="pager-btn">1</a>
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        ...
                        @else
                            <a href="{{ $designations->url($page) }}" class="pager-btn">{{ $page }}</a>
                        @endif
                    @endfor

                    @if ($end < $last)
                        @if ($end < $last - 1)<span class="pager-dots">&hellip;</span>@endif
                        <a href="{{ $designations->url($last) }}" class="pager-btn">{{ $last }}</a>
                    @endif

                    @if ($current >= $last)
                        <span class="pager-btn pager-btn-disabled" aria-disabled="true"><i class="bi bi-chevron-right"></i></span>
                    @else
                        <a href="{{ $designations->url($current + 1) }}" class="pager-btn" rel="next"><i class="bi bi-chevron-right"></i></a>
                    @endif
                </div>
            @endif
        </nav>
...
<script>
(function () {
    const input = document.getElementById('desSearchInput');
    const form = document.getElementById('desSearchForm');
...
function confirmDelete(id, name) {
    Swal.fire({
        ...
        text: `"${name}" will be removed. It can't be removed while School Management profiles or staff use it.`,
