...
<div class="wrap">
    ...
    <div class="header">
        <div>
            <h1>Departments</h1>
            <p>Manage the departments used to group classes (e.g. Science, Commerce, Arts).</p>
        </div>
        <a href="{{ route('admin.departments.create') }}" class="btn-save" style="text-decoration:none;">
    ...
    @php
        $sortLink = function (string $column, string $label) use ($sortBy, $sortDir, $search, $perPage) {
            ...
            $url = route('admin.departments', array_filter([
                'q'        => $search,
                'sort'     => $column,
                'dir'      => $nextDir,
    ...
    <div class="toolbar">
        <form action="{{ route('admin.departments') }}" method="GET" class="search-form" id="deptSearchForm">
            ...
            <input type="text" name="q" id="deptSearchInput" value="{{ $search }}"
                   placeholder="Search by name..." autocomplete="off">
            @if ($search !== '')
                <a href="{{ route('admin.departments', array_filter(['sort' => $sortBy !== 'name' ? $sortBy : null, 'dir' => $sortDir !== 'asc' ? $sortDir : null, 'per_page' => $perPage !== 10 ? $perPage : null])) }}" class="search-clear" title="Clear search">
        ...
        <div class="toolbar-right">
            <div class="toolbar-meta">
                {{ $items->total() }} {{ Str::plural('department', $items->total()) }}
                @if ($search !== '') for "<b>{{ $search }}</b>" @endif
            </div>

            <form action="{{ route('admin.departments') }}" method="GET" class="perpage-form" id="deptPerPageForm">
                @if ($search !== '') <input type="hidden" name="q" value="{{ $search }}"> @endif
                @if ($sortBy !== 'name') <input type="hidden" name="sort" value="{{ $sortBy }}"> @endif
                @if ($sortDir !== 'asc') <input type="hidden" name="dir" value="{{ $sortDir }}"> @endif
                <label for="deptPerPageSelect">Show</label>
                <select name="per_page" id="deptPerPageSelect" onchange="this.form.submit()">
        ...
        @if ($items->isEmpty())
            <div class="empty-state">
                <div class="ico-circle" style="width:52px;height:52px;margin:0 auto 12px;">
                    <i class="bi bi-diagram-3" style="color:#AEB4C4;font-size:22px;"></i>
                </div>
                @if ($search !== '')
                    <p>No departments match "<b>{{ $search }}</b>".</p>
                    <a href="{{ route('admin.departments') }}" class="choose-btn" style="display:inline-block;width:auto;padding:9px 20px;text-decoration:none;">Clear search</a>
                @else
                    <p>No departments yet.</p>
                    <a href="{{ route('admin.departments.create') }}" class="choose-btn" style="display:inline-block;width:auto;padding:9px 20px;text-decoration:none;">Add the first department</a>
                @endif
            </div>
        @else
                    ...
                    @foreach ($items as $item)
                        <tr>
                            ...
                            <td style="text-align:right;">
                                <a href="{{ route('admin.departments.edit', $item) }}" class="icon-btn" title="Edit">
    ...
    @if ($items->hasPages())
        @php
            $pageUrl = function (int $page) use ($search, $sortBy, $sortDir, $perPage) {
                return route('admin.departments', array_filter([
                    'q'        => $search,
...
(function () {
    ...
    const form = document.getElementById('deptSearchForm');
